...
def load_teacher_model(config):
    model_dir = "/local/mnt/workspace/kdwivedi/Knowledge_distillation/model"
    tokenizer_dir = "/local/mnt/workspace/kdwivedi/Knowledge_distillation/model/tokenizer"
    config_path = "/local/mnt/workspace/kdwivedi/Knowledge_distillation/model/config.json"
...
class SingleLayerTransformerDecoder(nn.Module):
    def __init__(self, config):
        ...
        # Token embeddings
        self.token_embeddings = nn.Embedding(config["vocab_size"], config["hidden_size"] // 8)

        # Position embeddings
        self.position_embeddings = nn.Embedding(config["max_position_embeddings"], config["hidden_size"] // 8)

        # Self-attention
        self.self_attn = nn.MultiheadAttention(
            embed_dim=config["hidden_size"] // 8,
        ...
        self.ff_network = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(config["hidden_size"] // 2, config["hidden_size"] // 8),
            nn.Dropout(0.1)
        )

        # Layer norms
        self.layer_norm1 = nn.LayerNorm(config["hidden_size"] // 8)
        self.layer_norm2 = nn.LayerNorm(config["hidden_size"] // 8)

        # Output layer
        self.output_layer = nn.Linear(config["hidden_size"] // 8, config["vocab_size"])
...
def evaluate_model(model, tokenizer, config,val_loader=None):
    model.eval()
    if val_loader is None:
        try:
            # Prepare test dataset
            test_dataset = TextDataset(tokenizer, config["test_data_path"], config["max_length"])
...
def train_with_knowledge_distillation(config):
    ...
    # Prepare dataset
    full_dataset = TextDataset(tokenizer, config["data_path"], config["max_length"])
...
def main():
    ...
    try:
        if args.mode == "train":
            ...
            evaluate_model(student_model, tokenizer, config)
            ...
            # Evaluate
            student_loss, student_ppl = evaluate_model(student_model, tokenizer, config)
            teacher_loss, teacher_ppl = evaluate_model(teacher_model, tokenizer, config)

            print(f"\nPerplexity comparison:")
            print(f"Student model perplexity: {student_ppl:.2f}")
            print(f"Teacher model perplexity: {teacher_ppl:.2f}")
            print(f"Relative performance: {(student_ppl / teacher_ppl):.2f}x  than teacher")
            ...
            prompts = [
                ...
                "What is happiness in this world and ",
                "Sun rises in the east and ",
                "Tea is getting cold, you   "
            ]
